...
cot::$db->registerTable('market_types');
...
function cot_build_structure_market_tree($parent = '', $selected = '', $level = 0, $template = '')
{
	...
	foreach ($children as $row)
	{
		...
		$t1->assign(array(
			...
			"ROW_SUBCAT" => ($subcats !== null && (count($subcats) > 0)) ? cot_build_structure_market_tree($row, $selected, $level + 1) : '',
			"ROW_LEVEL" => $level,
...
{
	global $db, $cot_extrafields, $cfg, $L, $Ls, $R, $db_market, $usr, $sys, $cot_yesno, $structure, $db_structure, $market_types;
	...
	if ($item_data['item_id'] > 0 && !empty($item_data['item_title']))
	{
		...
		$temp_array = array(
			...
			'TYPE' => $market_types[$item_data['item_type']],
			'TEXT' => $text,
			'SHORTTEXT' => $text_cut,
