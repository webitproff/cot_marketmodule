...
if ($a == 'add')
{
	...
	else
	{
		...
		cot_redirect(cot_url('market', 'm=add&c='.$c.'&type='.$type, '', true));
	}
}
...
$t->assign(array(
	...
	"PRDADD_FORM_TYPE" => (is_array($market_types)) ? cot_selectbox(($ritem['item_type']) ? $ritem['item_type'] : $cfg['market']['default_type'], 'rtype', array_keys($market_types), array_values($market_types)) : 'empty',
	"PRDADD_FORM_TYPETITLE" => (is_array($market_types) && !empty($type)) ? $market_types[$type] : '',
	"PRDADD_FORM_TITLE" => cot_inputbox('text', 'rtitle', $ritem['item_title'], 'size="56"'),
	'PRDADD_FORM_KEYWORDS' => cot_inputbox('text', 'rkeywords', $item['item_keywords'], array('size' => '32', 'maxlength' => '255')),
	'PRDADD_FORM_METATITLE' => cot_inputbox('text', 'rmetatitle', $item['item_metatitle'], array('size' => '64', 'maxlength' => '255')),
	'PRDADD_FORM_METADESC' => cot_textarea('rmetadesc', $item['item_metadesc'], 2, 64, array('maxlength' => '255')),
