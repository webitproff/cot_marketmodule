...
if (!empty($c))
{
	...
	$out['subtitle'] = (!empty($out['subtitle'])) ? $out['subtitle'] : $L['market_title'];
...
{
	$out['subtitle'] = (!empty($cfg['market']['cat___default']['metatitle'])) ? $cfg['market']['cat___default']['metatitle'] : $L['market_title'];
...
$catpatharray[] = array(cot_url('market'), $L['market_title']);
...
if(is_array($market_types)){
	foreach ($market_types as $i => $pr_type)
	{
		$t->assign(array(
			"PTYPE_ROW_ID" => $i,
			"PTYPE_ROW_TITLE" => $pr_type,
			"PTYPE_ROW_URL" => cot_url('market', 'c=' . $c . '&type=' . $i),
...
$t->assign(array(
	"PTYPE_ALL_URL" => cot_url('market', 'c=' . $c),
...
$t->assign(array(
	"SEARCH_ACTION_URL" => cot_url('market', "&type=" . $type, '', true),
